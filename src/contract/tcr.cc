...
// Registry::Registry(const Address& registry_id)
//     : UpgradableContract(registry_id, ContractID::Registry)

// {
// }

// void Registry::init(const Address& _token_id, const Address& _voting_id,
...
//   params.init(_vote_quorum, _dispensation_percentage, _min_deposit,
...
//   listing_nonce = +listing_nonce + 1;
//   if (m_listings[+listing_nonce].exist())
//     throw Failure("This nonce already have data.");
//   Listing& listing = m_listings[+listing_nonce];
//   listing.init(+params.apply_duration + Global::get().block_time,
//   get_sender(),
//                token_deposit, data);
...
//   assert_con(get_sender() == +m_listings[list_id].owner,
//              "You aren't owner's item.");

//   m_listings[list_id].unstake_deposit =
//       +m_listings[list_id].unstake_deposit + token_deposit;
...
//   assert_con(get_sender() == +m_listings[list_id].owner,
//              "You aren't owner's item.");
//   assert_con(token_withdraw <= +m_listings[list_id].unstake_deposit,
//              "Not enough unstake deposit to withdraw.");
//   assert_con(+m_listings[list_id].unstake_deposit - token_withdraw >=
//                  +params.min_deposit,
//              "Not enough remained unstake deposit to stake on next
//              challenge.");

//   m_listings[list_id].unstake_deposit =
//       +m_listings[list_id].unstake_deposit - token_withdraw;
...
// {
//   assert_con(list_exist(list_id), "Item not found.");
//   Listing& listing = m_listings[list_id];
...
//   if (m_challenges[+challenge_nonce].exist())
//     throw Failure("This nonce already have data.");
...
//   Challenge& cha = m_challenges[+challenge_nonce];
//   cha.init(poll_id, get_sender(), data, reward, min_deposit);

//   listing.challenge_id = +challenge_nonce;
//   listing.unstake_deposit = +listing.unstake_deposit - min_deposit;
...
//   assert_con(+m_challenges[challenge_id].is_resolved,
//              "This challenge hasn't resolved yet.");

//   auto& vote = Global::get().m_ctx->get<Voting>(+voting_id);
//   uint256_t voter_tokens = vote.get_number_pass_token(
//       sender, +m_challenges[challenge_id].poll_id, salt);
//   uint256_t reward = voter_reward(sender, challenge_id, salt);
//   m_challenges[challenge_id].remaining_winning_token =
//       +m_challenges[challenge_id].remaining_winning_token - voter_tokens;
//   m_challenges[challenge_id].remaining_reward_pool =
//       +m_challenges[challenge_id].remaining_reward_pool - reward;

//   m_challenges[challenge_id].token_claimed[sender] = true;
...
//   const Challenge& cha = m_challenges[challenge_id];
//   uint256_t remain_total_token = +cha.remaining_winning_token;
//   uint256_t remain_reward = +cha.remaining_reward_pool;
//   auto& vote = Global::get().m_ctx->get<Voting>(+voting_id);

//   uint256_t voter_tokens =
//       vote.get_number_pass_token(voter, +cha.poll_id, salt);
...
//   return (+m_listings[list_id].app_expire_time < Global::get().block_time &&
...
// {
//   assert_con(list_exist(list_id), "Item not found.");
//   return +m_listings[list_id].is_listed;
// }
...
//   uint256_t challenge_id = +m_listings[list_id].challenge_id;
//   return (challenge_id > 0 && !(+m_challenges[challenge_id].is_resolved));
// }
...
//   auto& vote = Global::get().m_ctx->get<Voting>(+voting_id);
//   uint256_t challenge_id = +m_listings[list_id].challenge_id;
//   return vote.is_poll_ended(+m_challenges[challenge_id].poll_id);
// }
...
// {
//   assert_con(list_exist(list_id), "Item not found.");
//   return +m_listings[list_id].data;
// }

// uint256_t Registry::get_deposit(uint256_t list_id) const
// {
//   assert_con(list_exist(list_id), "Item not found.");
//   return +m_listings[list_id].unstake_deposit;
// }
...
//     return +m_listings[list_id].challenge_id;
//   return 0;
// }
...
//   return +m_challenges[challenge_id].poll_id;
// }
...
//   Challenge& cha = m_challenges[challenge_id];
...
//     m_listings[list_id].unstake_deposit =
//         +m_listings[list_id].unstake_deposit + reward;
//   } else {
//     reset_listing(list_id);

//     set_sender();
//     auto& token = Global::get().m_ctx->get<Token>(+token_id);
//     token.transfer(+cha.challenger_address, reward);
//   }
...
// {
//   m_listings[list_id].is_listed = true;
// }
...
// {
//   return m_listings[list_id].exist();
// }

// bool Registry::challenge_exist(const uint256_t& challenge_id) const
// {
//   return m_challenges[challenge_id].exist();
// }
